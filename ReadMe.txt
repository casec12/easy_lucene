...
4. BusBean到DocBean的转换依赖BeanUtil，因此BusBean需要属性的getter、setter，DocBean需要getter、setter方法，还需要公有无参构造方法
5. DocBean只保留需要的属性即可
5. 配置Bean映射的特殊关系，修改文件with_prototype.easy_lucene.config.Constants类,其中：
...
6. 运行with_prototype.easy_lucene.example.Example例子，例子中包含了所有的服务方法调用。记录索引、更新索引、删除索引、索引查询